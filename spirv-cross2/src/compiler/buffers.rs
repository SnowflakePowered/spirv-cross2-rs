...
impl<'a, T> Compiler<'a, T> {
    ...
    pub fn active_buffer_ranges(&self, handle: Handle<VariableId>) -> error::Result<&'a [BufferRange]> {
        let handle = self.yield_id(handle)?;

        unsafe {
            let mut ranges = std::ptr::null();
            let mut size = 0;
            sys::spvc_compiler_get_active_buffer_ranges(self.0.as_ptr(), handle, &mut ranges, &mut size)
                .ok(self)?;
...
mod test {
    ...
    pub fn get_active_buffer_ranges() -> Result<(), SpirvCrossError> {
        ...
        let ubo: Vec<_> = compiler.shader_resources()?
